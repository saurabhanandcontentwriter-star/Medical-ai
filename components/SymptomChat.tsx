...
const SymptomChat: React.FC<SymptomChatProps> = ({ chatMessages, setChatMessages }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const responseText = await sendMessageToGemini(chatMessages, languagePrompt);
    ...
    <div className="flex flex-col h-full bg-white md:bg-gray-50 dark:bg-gray-900 max-h-[calc(100vh-80px)] md:max-h-screen">
      <div className="p-4 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 md:bg-transparent md:border-none md:pt-6 md:px-6 flex justify-between items-center">
        ...
        <div className="flex bg-gray-100 dark:bg-gray-700 p-1 rounded-[1rem] shadow-inner border border-gray-200 dark:border-gray-600">
           <button onClick={() => setVoiceLanguage('English')} className={`px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${voiceLanguage === 'English' ? 'bg-teal-600 text-white shadow-md' : 'text-gray-400'}`}>English</button>
           <button onClick={() => setVoiceLanguage('Hindi')} className={`px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${voiceLanguage === 'Hindi' ? 'bg-teal-600 text-white shadow-md' : 'text-gray-400'}`}>हिंदी</button>
        </div>
      </div>
      ...
      <div className="p-4 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 md:m-4 md:rounded-[2rem] md:shadow-xl md:border">
        <div className="flex items-end space-x-2">
          <textarea value={inputText} onChange={(e) => setInputText(e.target.value)} onKeyDown={handleKeyPress} placeholder={voiceLanguage === 'Hindi' ? "अपने लक्षणों का वर्णन करें..." : "Describe your symptoms..."} className="flex-1 bg-gray-50 dark:bg-gray-700 border-0 rounded-2xl px-5 py-4 text-gray-800 dark:text-white placeholder-gray-400 focus:ring-2 focus:ring-teal-500 focus:bg-white transition-all resize-none max-h-32 font-medium" rows={1} />
