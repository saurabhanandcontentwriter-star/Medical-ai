    ...
    return (
      <div className="p-6 h-full flex flex-col items-center justify-center text-gray-500">
        <div className="w-24 h-24 bg-gray-100 rounded-full flex items-center justify-center mb-6">
          <svg className="w-12 h-12 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>
        </div>
        <h2 className="text-xl font-bold text-gray-900 mb-2">No Active Orders</h2>
        <p className="text-center max-w-xs">Your medicine orders and lab test bookings will appear here.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 pb-24 md:pb-6 max-w-5xl mx-auto w-full overflow-y-auto">
      <header className="mb-8">
        ...
        <p className="text-gray-500">Real-time status of your medicines and appointments.</p>
      </header>
          ...
          <div key={order.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 transition-all hover:shadow-md">
            ...
            <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
              <div className="flex items-start gap-4">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${order.type === 'medicine' ? 'bg-teal-50 text-teal-600' : 'bg-blue-50 text-blue-600'}`}>
                  {order.type === 'medicine' ? (
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">Order ID: #{order.id} • {order.date.toLocaleDateString()}</p>
                </div>
              </div>
            ...
            <div className="relative mb-6">
              ...
              <div className="hidden md:flex justify-between items-center relative z-10 px-4">
                ...
                <div className="absolute left-0 right-0 top-3 h-0.5 bg-gray-200 -z-10 mx-8"></div>
                {/* Progress Line */}
                 <div 
                ...
                {order.steps.map((step, idx) => (
                  <div key={idx} className="flex flex-col items-center">
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center bg-white transition-colors duration-300 ${
                      step.isCompleted ? 'border-teal-500 bg-teal-500' : 'border-gray-300'
                    }`}>
                      {step.isCompleted && (
                    ...
                    <div className="text-center mt-2">
                      <p className={`text-xs font-bold ${step.isCompleted ? 'text-teal-700' : 'text-gray-400'}`}>{step.label}</p>
                      {step.timestamp && <p className="text-[10px] text-gray-400">{step.timestamp}</p>}
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  <div key={idx} className="relative flex items-start pb-6 last:pb-0">
                    ...
                    {idx !== order.steps.length - 1 && (
                      <div className={`absolute left-[11px] top-6 bottom-0 w-0.5 ${
                        step.isCompleted && order.steps[idx + 1].isCompleted ? 'bg-teal-500' : 'bg-gray-200'
                      }`}></div>
                    )}
                    
                    <div className={`w-6 h-6 rounded-full border-2 flex-shrink-0 flex items-center justify-center bg-white z-10 ${
                      step.isCompleted ? 'border-teal-500 bg-teal-500' : 'border-gray-300'
                    }`}>
                      {step.isCompleted && (
                        <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                    ...
                    <div className="ml-4">
                      <p className={`text-sm font-bold ${step.isCompleted ? 'text-teal-700' : 'text-gray-400'}`}>{step.label}</p>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                {order.reportUrl && (
                  <button 
                    ...
                    className="flex items-center px-4 py-2.5 bg-teal-50 text-teal-700 rounded-xl text-sm font-medium hover:bg-teal-100 transition-colors border border-teal-100"
                  >
                ...
                {order.invoiceUrl && (
                  <button 
                    ...
                    className="flex items-center px-4 py-2.5 bg-gray-50 text-gray-700 rounded-xl text-sm font-medium hover:bg-gray-100 transition-colors border border-gray-200"
                  >
            ...
            {order.deliveryAgent && (
              <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between bg-gray-50 p-4 rounded-xl">
                <div className="flex items-center space-x-3">
                   ...
                   <div>
                     <p className="text-sm font-bold text-gray-900">{order.type === 'medicine' ? 'Delivery Partner' : 'Lab Technician'}</p>
                     <p className="text-sm text-gray-600">{order.deliveryAgent.name}</p>
                   </div>
                </div>
                <div className="flex items-center">
                   <a href={`tel:${order.deliveryAgent.phone}`} className="flex items-center text-teal-700 bg-white border border-teal-200 hover:bg-teal-50 px-3 py-2 rounded-lg transition-colors shadow-sm">
      ...
      {previewDoc && (
        <div className="fixed inset-0 z-[100] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="bg-gray-800 text-white p-4 flex justify-between items-center">
              <h3 className="text-lg font-bold flex items-center">
                {previewDoc.type === 'invoice' ? (
                   <><svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>Invoice Preview</>
                ) : (
                  ...
                  <table className="w-full mb-8">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td className="py-4 text-right font-bold text-xl text-teal-600">₹{previewDoc.order.amount}</td>
                      </tr>
                    </tbody>
                  </table>
                ) : (
            ...
            {/* Modal Actions */}
            <div className="bg-white p-4 border-t border-gray-100 flex justify-end gap-3">
              <button 
              ...
              <button 
                ...
                className="px-6 py-2.5 rounded-xl bg-teal-600 text-white font-medium hover:bg-teal-700 transition-colors flex items-center shadow-lg shadow-teal-200"
              >
