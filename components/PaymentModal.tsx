...
const PaymentModal: React.FC<PaymentModalProps> = ({ amount, isOpen, onClose, onSuccess, title = "Complete Payment" }) => {
  ...
  return (
    <div className="fixed inset-0 z-[150] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden relative flex flex-col max-h-[90vh]">
        ...
        <div className="bg-gray-50 p-6 border-b border-gray-100 flex justify-between items-center">
          ...
          <button onClick={onClose} disabled={step !== 'select'} className="text-gray-400 hover:text-gray-600 disabled:opacity-30">
          ...
          {step === 'select' && (
            <div className="space-y-6">
              <div className="text-center py-4 bg-teal-50 rounded-xl border border-teal-100">
                ...
                <p className="text-3xl font-bold text-teal-700">₹{amount}</p>
              </div>

              <div className="space-y-3">
                <p className="font-medium text-gray-700">Select Payment Method</p>
                
                <label className={`flex items-center p-4 border rounded-xl cursor-pointer transition-all ${paymentMethod === 'upi' ? 'border-teal-500 bg-teal-50/50' : 'border-gray-200 hover:border-gray-300'}`}>
                  ...
                  <svg className="w-6 h-6 text-teal-600 opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
                </label>

                <label className={`flex items-center p-4 border rounded-xl cursor-pointer transition-all ${paymentMethod === 'card' ? 'border-teal-500 bg-teal-50/50' : 'border-gray-200 hover:border-gray-300'}`}>
                  ...
                  <svg className="w-6 h-6 text-teal-600 opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>
                </label>

                <label className={`flex items-center p-4 border rounded-xl cursor-pointer transition-all ${paymentMethod === 'cod' ? 'border-teal-500 bg-teal-50/50' : 'border-gray-200 hover:border-gray-300'}`}>
                  ...
                  <svg className="w-6 h-6 text-teal-600 opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                </label>
              </div>

              <button 
                onClick={handlePay}
                className="w-full bg-teal-600 text-white py-4 rounded-xl font-bold text-lg hover:bg-teal-700 transition-all shadow-lg shadow-teal-200 mt-4"
              >
                Pay ₹{amount}
              </button>
            </div>
